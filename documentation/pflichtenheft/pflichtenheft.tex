...
\textit{Synchrony}  ist eine Software, die es ermöglicht, Verzeichnisse und Dateien zu synchronisieren.
Die Anwendung ermöglicht Datenbestände zwischen verschiedenen Geräten synchron zu halten.
\textit{Synchrony} stellt eine benutzerfreundliche Benutzeroberfläche zur Verfügung. Diese dient dazu
zu synchronsierende Ordner anzugeben und den Synchronisationsdienst zu aktivieren. Ist der Dienst auf
den jeweiligen Endgeräten aktiv, so läuft der Abgleich der Daten im Hintergrund ab. Die Verzeichnisse 
...
\begin{itemize}
\item Der Benutzer kann Dateien zwischen zwei oder mehreren Rechnern synchronisieren. Die Daten werden hierbei repliziert.
\item Der Benutzer kann den Synchronisationsdienst konfigurieren.
\item Bei der Installation wird ein Account vergeben, der zur Identifikation eines Nutzers im Netzwerk dient.
...
\item Nach einmaliger Konfiguration ist der Dienst funktions- und synchronisationsbereit.
...
Prinzipiell ist die Software für jeden Benutzertyp geeignet, der Dateien zwischen mehreren System synchron halten bzw. den Dienst zur Datensicherung verwenden will.\par
...
\begin{itemize}
	...
	\item Buchautoren: Skripte, Notizen usw. können zwischen verschiedenen Geräten an verschiedenen Standorten synchronisiert und repliziert werden. 
\end{itemize}
\subsection{Betriebsbedingungen}
Der Dateisynchronisationsdienst soll i.d.R. im Hintergrund als Benutzerdienst laufen und im selbsstständig aktiv sein. Der Dienst wird daher i.d.R. beim Einloggen des Anwenders automatisch gestartet.\par
...
\subsection{Software}
\textit{Synchrony}  stellt folgende Anforderungen an das System hinsichtlich softwareseitiger Unterstützung:
\begin{itemize}
	\item Java Runtime Environment 7 oder höher\footnote{neue Möglichkeiten der Dateisystem-Programmierschnittstellen sollen genutzt werden.\newline Siehe hierzu die offiziellen Sun-Webnotes: \url{http://java.sun.com/javase/7/webnotes/index.html}}
	\item ein mit der Dateisystem-API von Java 7 kompatibles Dateisystem
	\item für die Verwendung der graphischen Schnittstelle (GUI) muss ein kompatibles Betriebssystem bzw. ein kompatibler Window-Manager vorhanden sein
\end{itemize}
\subsection{Hardware}
...
\textit{Synchrony}  gleicht diese Daten miteinander ab, und bringt diese auf einen einheitlichen Stand. Bei einem nicht
...
\item Einstellungen\footnote{Beispiele: Maximale zu verwendende Bandbreite, Auswahl der zu synchronisierenden Ordner ...}
\item Statusinfo
\end{itemize}
...
\glqq Normaluser\grqq\ eine mit möglichst kurzer Einarbeitungszeit bedienbare Schnittstelle anzubieten.
...
\begin{itemize}
    ...
    \item ökonomischen Festplattenzugriff (um Systemgesamtleistung nicht zu beinflussen),
...
wird ein ausblendbares Tray-Icon angeboten, über das der Benutzer auch den Dienst
...
\newpage
\section{Testfälle}
Die Testfälle werden jeweils vor der Programmierung einer Funktion entworfen und implementiert. Das Prinzip dahinter ist das sogenannte
...
\subsection{Software}
Für die Entwicklung des Synchronisations-Tools \textit{Synchrony} wird die Entwicklungsumgebung NetBeans (Version 6.8) verwendet.
\textit{Synchrony} wird in der Programmiersprache Java entwickelt - dazu wird auf den Entwicklungsrechnern JDK 7 installiert.
...
\subsection{Hardware}
Die Entwicklung erfolgt auf unterschiedlichen Rechnern. Als Entwickler-Betriebssystem werden verschiedene Linuxdistributionen verwendet. Vorrangig wird die Distribution Ubuntu in ihren Releases 9.04, 9.10 und 10.04 eingesetzt..
